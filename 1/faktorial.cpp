...
int main() {
  ...
  vector<Instruction> instrukce(
      {
        //Do promenne x ulozime cislo, ktereho faktorial chceme spocitat
        //vysledek bude po konci programu na stacku a vyprinti se
        //pokud je x zaporne, nevypise se nic (nedefinovano matematicky, ze)
        //v pripade nezaporneho x se vypise vysledek
        //12 je s 32 bit inty nejvyssi cislo, kde to jeste nepretece
        Instruction{.op = OP_PUSH, .value = 12}, //0 cislo radku
        Instruction{.op = OP_STORE, .value = "x"}, //1

        Instruction{.op = OP_LOAD, .value = "x"}, //2
        Instruction{.op = OP_PUSH, .value = 0}, //3
        Instruction{.op = OP_LT}, //4
        Instruction{.op = OP_BRANCH, .value = 32}, //5

        //jestli je x 1 return 1
        Instruction{.op = OP_LOAD, .value = "x"}, //6
        Instruction{.op = OP_PUSH, .value = 1}, //7
        Instruction{.op = OP_EQ}, //8
        Instruction{.op = OP_BRANCH, .value = 15}, //9
        Instruction{.op = OP_NOP}, //10

        //jestli je x 0 return 1
        Instruction{.op = OP_LOAD, .value = "x"}, //11
        Instruction{.op = OP_PUSH, .value = 0}, //12
        Instruction{.op = OP_EQ}, //13
        Instruction{.op = OP_NOT}, //14
        Instruction{.op = OP_BRANCH, .value = 19}, //15
        //tady je cast co provadi print 1
        Instruction{.op = OP_PUSH, .value = 1}, //16
        Instruction{.op = OP_PRINT}, //17
        Instruction{.op = OP_PUSH, .value = 1}, //18
        Instruction{.op = OP_BRANCH, .value = 32}, //19


        //nacteni cisla z x na zasobnik (=protoze jeste na zasobniku nemame predchozi cislo, ktere budeme nasobit)
        Instruction{.op = OP_LOAD, .value = "x"}, //20

        //vypocet x-1 
        Instruction{.op = OP_LOAD, .value = "x"}, //21
        Instruction{.op = OP_PUSH, .value=1}, //22
        Instruction{.op = OP_SUB}, //23

        //x = x-1
        Instruction{.op = OP_STORE, .value = "x"}, //24

        Instruction{.op = OP_LOAD, .value = "x"}, //25
        
        Instruction{.op = OP_MUL}, //26
        Instruction{.op = OP_NOP}, //27 //OP_PRINT

        //if x > 1 jump up and repeat the loop
        Instruction{.op = OP_PUSH, .value = 1}, //28
        Instruction{.op = OP_LOAD, .value = "x"}, //29
        Instruction{.op = OP_LT}, //30
        Instruction{.op= OP_BRANCH, .value = 20}, //31
        Instruction{.op= OP_PRINT}, //32
